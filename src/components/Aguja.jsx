...
function Aguja({ latamMode }) {
  ...
  const agujaText = ["🇦🇷: Aguja.", "🇧🇷: Acém.", "🇪🇸: Morrillo.", " 🇫🇷: Basses côtes.", "🇺🇸: Chuck."];
  const agujaTextLatam = [
    "🇨🇱: Huachalomo.",
    "🇨🇴: Lomo de Aguja.",
    "🇪🇨: Lomo de Aguja.",
    "🇲🇽: Tapa de Lomo.",
  ...
  return (
    <ClickAwayListener onClickAway={handleTooltipClose}>
      <CustomTooltip
        title={
          <ul>{latamMode ? agujaTextLatam.map((str) => <p>{str} </p>) : agujaText.map((str) => <p>{str} </p>)}</ul>
        }
