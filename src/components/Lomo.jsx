...
function Lomo({ latamMode }) {
  ...
  const lomoText = ["🇦🇷: Lomo.", "🇧🇷: Filé Mignon.", "🇪🇸: Solomillo.", " 🇫🇷: Fillet.", "🇺🇸: Tenderloin."];
  const lomoTextLatam = ["🇨🇱: Filete.", "🇨🇴: Lomo Fino.", "🇪🇨: Lomo Fino.", "🇲🇽: Fileta.", "🇵🇪: Lomo.", "🇺🇾: Lomo.", "🇻🇪: Lomito."];
  return (
    <ClickAwayListener onClickAway={handleTooltipClose}>
      <CustomTooltip
        title={<ul>{latamMode ? lomoTextLatam.map((str) => <p>{str} </p>) : lomoText.map((str) => <p>{str} </p>)}</ul>}
