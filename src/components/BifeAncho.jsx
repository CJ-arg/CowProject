...
const BifeAncho = ({ latamMode }) => {
  ...
  const bifeAnchoText = ['🇦🇷: Bife Ancho.',
    '🇧🇷: Capa de filé.',
    '🇪🇸: Lomo ALto.',
    ' 🇫🇷: Côtes, Entrecôtes.', '🇺🇸: Fore Rib.'];
  const bifeAnchoTextLatam = ['🇨🇱: Lomo Liso.',
    '🇨🇴: Lomo Ancho.',
    '🇪🇨: Lomo de Falda.', '🇲🇽: Lomo.',
    '🇵🇪: Churrasco largo redondo.', '🇺🇾: Espinazo con Lomo.', '🇻🇪: Solomo de Cuerito.'];
  return (
    <ClickAwayListener onClickAway={handleTooltipClose}>
      <CustomTooltip
        title={(
          <ul>
            {latamMode ? bifeAnchoTextLatam.map((str) => (
              <p>
                {str}
                {' '}
              </p>
            )) : bifeAnchoText.map((str) => (
              <p>
                {str}
                {' '}
              </p>
