...
function Cuello({ latamMode }) {
  ...
  const cuelloText = ["🇦🇷: Cogote.", "🇧🇷: Pescoço.", "🇪🇸: Pescuezo.", " 🇫🇷: Collier.", "🇺🇸: Neck and Sticking."];
  const cuelloTextLatam = ["🇨🇱: Cogote.", "🇨🇴: Cogote.", "🇪🇨: Nuca.", "🇲🇽: Pescuezo.", "🇵🇪: Pescueso.", "🇺🇾: Cogote.", "🇻🇪: Pescuezo."];
  return (
    <ClickAwayListener onClickAway={handleTooltipClose}>
      <CustomTooltip
        title={<ul>{latamMode ? cuelloTextLatam.map((str) => <p>{str} </p>) : cuelloText.map((str) => <p>{str} </p>)}</ul>}
