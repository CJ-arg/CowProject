...
function Cuadrada({ latamMode }) {
  ...
  const cuadradaText = ["🇦🇷: Cuadrada.", "🇧🇷: Alcatra.", "🇪🇸: Contra.", " 🇫🇷: Tranche Ronde.", "🇺🇸: Top Sirloin."];
  const cuadradaTextLatam = [
    "🇨🇱: Ganso.",
    "🇨🇴: Bota.",
    "🇪🇨: Pulpa Blanca.",
    "🇲🇽: Contra Cara.",
  ...
  return (
    <ClickAwayListener onClickAway={handleTooltipClose}>
      <CustomTooltip
        title={<ul>{latamMode ? cuadradaTextLatam.map((str) => <p>{str} </p>) : cuadradaText.map((str) => <p>{str} </p>)}</ul>}
