...
function Matambre({ latamMode }) {
  ...
  const matambreText = ["🇦🇷: Matambre.", "🇧🇷: Matambre.", "🇪🇸: Aleta.", " 🇫🇷: (Pas de Nom).", "🇺🇸: Rose Meat."];
  const matambreTextLatam = ["🇨🇱: Malaya.", "🇨🇴: Sobre Barriga.", "🇪🇨: Falda Dorada.", "🇲🇽: Fresada.", "🇵🇪: Malaya.", "🇺🇾: Matambre.", "🇻🇪: Pollo de Res."];
  return (
    <ClickAwayListener onClickAway={handleTooltipClose}>
      <CustomTooltip
        title={<ul>{latamMode ? matambreTextLatam.map((str) => <p>{str} </p>) : matambreText.map((str) => <p>{str} </p>)}</ul>}
