...
function Peceto({ latamMode }) {
  ...
  const pecetoText = ["🇦🇷: Peceto.", "🇧🇷: Lagarto.", "🇪🇸: Redondel.", " 🇫🇷: Tranche.", "🇺🇸: Tranche."];
  const pecetoTextLatam = ["🇨🇱: Pollo Ganso.", "🇨🇴: Muchacho.", "🇪🇨: Salón.", "🇲🇽: Custe.", "🇵🇪: Pejerrey.", "🇺🇾: Pulpa Chorizo.", "🇻🇪: Muchacho Redondo."];

  return (
    <ClickAwayListener onClickAway={handleTooltipClose}>
      <CustomTooltip
        title={<ul>{latamMode ? pecetoTextLatam.map((str) => <p>{str} </p>) : pecetoText.map((str) => <p>{str} </p>)}</ul>}
