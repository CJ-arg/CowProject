...
function Pecho({ latamMode }) {
  ...
  const pechoText = ["🇦🇷: Pecho.", "🇧🇷: Peito.", "🇪🇸: Pecho.", " 🇫🇷: Poitrine Gros Bout.", "🇺🇸: Brisket."];
  const pechoTextLatam = ["🇨🇱: Tapa Pecho.", "🇨🇴: Pecho.", "🇪🇨: Pecho.", "🇲🇽: Pecho.", "🇵🇪: Pecho.", "🇺🇾: Pecho.", "🇻🇪: Pecho."];
  return (
    <ClickAwayListener onClickAway={handleTooltipClose}>
      <CustomTooltip
        title={<ul>{latamMode ? pechoTextLatam.map((str) => <p>{str} </p>) : pechoText.map((str) => <p>{str} </p>)}</ul>}
