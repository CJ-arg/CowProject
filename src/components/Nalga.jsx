...
function Nalga({ latamMode }) {
  ...
  const nalgaText = ["🇦🇷: Nalga.", "🇧🇷: Coxae Mole Cha.", "🇪🇸: Tapa.", " 🇫🇷:  Tende de Tranche.", "🇺🇸: Topside."];
  const nalgaTextLatam = [
    "🇨🇱: Posta Negra.",
    "🇨🇴: Centro de pierna.",
    "🇪🇨: Pulpa Negra.",
  ...
  return (
    <ClickAwayListener onClickAway={handleTooltipClose}>
      <CustomTooltip
        title={<ul>{latamMode ? nalgaTextLatam.map((str) => <p>{str} </p>) : nalgaText.map((str) => <p>{str} </p>)}</ul>}
