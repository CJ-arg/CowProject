...
const ColitaDeCuadril = ({ latamMode }) => {
  const colitaDeCuadrilText = ['🇦🇷: Colita De Cuadril.',
    '🇧🇷: Maminha de Alcatra.',
    '🇪🇸: Rabillo de Cadera.',
    '🇫🇷: Bavette d\' Aloyau.', '🇺🇸: Tri Tip.'];
  const colitaDeCuadrilTextLatam = ['🇨🇱: Punta de picana.',
    '🇨🇴: Colita de Cadera.',
    '🇪🇨: Pajarilla.', '🇲🇽: Empuje.',
    '🇵🇪: Bistec de Cadera .', '🇺🇾: Cuadril.', '🇻🇪: Punta de trasero.'];

  return (
    <CustomTooltip
      title={(
        <ul>
          {latamMode ? colitaDeCuadrilTextLatam.map((str) => (
            <p>
              {str}
              {' '}
            </p>
          )) : colitaDeCuadrilText.map((str) => (
            <p>
              {str}
              {' '}
            </p>
