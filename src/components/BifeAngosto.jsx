...
const BifeAngosto = ({ latamMode }) => {
  ...
  const bifeAngostoText = ['🇦🇷: Bife Angosto.',
    '🇧🇷: Filé de costela.',
    '🇪🇸: Lomo Bajo.',
    ' 🇫🇷: Faux Filet.', '🇺🇸: Strip Loin.'];
  const bifeAngostoTextLatam = ['🇨🇱: Lomo Veteado.',
    '🇨🇴: Lomo Ancho.',
    '🇪🇨: Lomo de Asado.', '🇲🇽: Lomo.',
    '🇵🇪: Churrasco largo redondo.', '🇺🇾: Espinazo sin Lomo.', '🇻🇪: Solomo de Cuerito.'];
  return (
    <ClickAwayListener onClickAway={handleTooltipClose}>
      <CustomTooltip
        title={(
          <ul>
            {latamMode ? bifeAngostoTextLatam.map((str) => (
              <p> {str} </p>
            )) : bifeAngostoText.map((str) => (
              <p> {str}</p>
            ))}
          </ul>
        )}
