...
function BifeAngosto({ latamMode }) {
  ...
  const bifeAngostoText = ["🇦🇷: Bife Angosto.", "🇧🇷: Filé de costela.", "🇪🇸: Lomo Bajo.", " 🇫🇷: Faux Filet.", "🇺🇸: Strip Loin."];
  ...
  return (
    <ClickAwayListener onClickAway={handleTooltipClose}>
      <CustomTooltip
        title={<ul>{latamMode ? bifeAngostoTextLatam.map((str) => <p> {str} </p>) : bifeAngostoText.map((str) => <p> {str}</p>)}</ul>}
